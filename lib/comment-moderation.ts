...
export async function verifyHuman(token:unknown,hostname:string,action='comment'){
  ...
  if(!result.success||result.hostname!==hostname||result.action!==action)throw new CommentCheckError('The anti-bot check expired or failed. Please try again.',400);
}
