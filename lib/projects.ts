...
export type DevLogEntry = { id:string; project_id:string; title:string; slug:string; entry_date:string; version:string|null; tag:string|null; summary:string; content:string; github_url:string|null; media:Record<string,unknown>[]; code:Record<string,unknown>[]; status:'draft'|'published'; created_at:string; updated_at:string };
