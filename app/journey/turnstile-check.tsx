...
export default function TurnstileCheck({siteKey,onToken,revision}:{siteKey:string;onToken:(token:string)=>void;revision:number}){
  ...
  useEffect(()=>{
    ...
    loadScript().then(()=>{
      ...
      setError('');
      widget=window.turnstile.render(container.current,{sitekey:siteKey,action:'comment',theme:'auto',size:'flexible',callback:(token:string)=>onToken(token),'expired-callback':()=>onToken(''),'error-callback':()=>{onToken('');setError('The anti-bot check could not complete. Reload to retry.');}});
